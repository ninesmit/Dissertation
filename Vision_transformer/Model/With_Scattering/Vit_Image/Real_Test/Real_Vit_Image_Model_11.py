...
class Scattering2dVIT(nn.Module):
    ...
    def __init__(self, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim, pool, channels, dim_head, dropout, emb_dropout):
        ...
        self.image_size = int((image_size/2) ** 2)
...
for epoch in range(0, num_epoch):
    ...
    with open(text_file_name, 'a') as file:
        file.write(f"""\nEpoch: {epoch+1}\nTraining loss: {[train_loss_log[0][i] for i in range(15)]}\nTraining time: {train_time}\nAverage test loss:{test_loss_log}\nTest time: {test_time}\nAccuracy: {test_accuracy}\nHighest Acc:{acc_log.max()} at epoch {np.argmax(acc_log) + 1}""")

    # Save the model every 20 epochs
    if (epoch + 1) % 100 == 0:
        torch.save(model.state_dict(), f'Real_Vit_Img_Model11_epoch_{epoch+1}.pth')
