...
class Scattering2dVIT(nn.Module):
    ...
    '''
    def __init__(self, scale, image_size, num_patches, num_classes, dim, depth, heads, mlp_dim, pool, channels, dim_head, dropout, emb_dropout):
    ...
    def build(self):
        ...
        patch_dim = int((image_size/(2**self.scale)) ** 2)
...
mode = 3
image_size = 128
text_file_name = 'Vit_Scat_Model4.txt'
num_classes = 10
dim = 1024
depth = 10
heads = 10
...
model = Scattering2dVIT(scale=mode,
...
for epoch in range(0, num_epoch):
    ...
    if (epoch + 1) % 10 == 0:
        torch.save(model.state_dict(), f'Vit_Scat_Model4_epoch_{epoch+1}.pth')
