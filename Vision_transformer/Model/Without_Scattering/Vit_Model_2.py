...
text_file_name = 'Vit_Model2.txt'
use_cuda = torch.cuda.is_available()
device = torch.device("cuda" if use_cuda else "cpu")

scattering = scattering.to(device)
model = Scattering2dVIT(image_size=128, patch_size=16, num_classes=10, dim=1024, depth=5, heads=5, mlp_dim=512, dropout=0.15, emb_dropout=0.15).to(device)
...
for epoch in range(0, num_epoch):
    ...
    if (epoch + 1) % 10 == 0:
        torch.save(model.state_dict(), f'ViT_Model2_epoch_{epoch+1}.pth')
